 ...
 */
@WebServlet(name = "SolicitudPem", urlPatterns = {"/SolicitudPem"})

public class SolicitudPem extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
        String correoMed = request.getParameter("user_mail");
        SolicitudPadre soli = new SolicitudPadre();
        ActsUser act = new ActsUser();
        String id_usuario = "";
        try {
            id_usuario = act.Buscar(correoMed);
        } catch (Exception ex) {
            System.out.println("Error al buscar usuario con el correo med");
            Logger.getLogger(SolicitudPem.class.getName()).log(Level.SEVERE, null, ex);
        }
        String id_medico = "";
        try {
            id_medico = act.BuscarDRUsuario(id_usuario);
        } catch (Exception ex) {
            System.out.println("Error al buscar medico con su id de usuario");
            Logger.getLogger(SolicitudPem.class.getName()).log(Level.SEVERE, null, ex);
        }
        if (id_medico == null) {
            ...
            out.println("alert('No existe ese médico.')");
            ...
            String id_usr_padre = "";
            try {
                id_usr_padre = act.BuscarSesion(correoOrig);
            } catch (Exception ex) {
                Logger.getLogger(SolicitudPem.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            String id_persona = "";
            try {
                id_persona = act.BuscarPadreusuario(id_usr_padre);
            } catch (Exception ex) {
                Logger.getLogger(SolicitudPem.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            if (!existe) {
                try {
                    mandado = soli.mandarSoliPadre(id_persona, id_medico);
                } catch (Exception ex) {
                    System.out.println("Error al buscar medico con su id de usuario");
                    Logger.getLogger(SolicitudPem.class.getName()).log(Level.SEVERE, null, ex);
                }
