 ...
 *
 */
@WebServlet(name = "ActualizarUsrMed", urlPatterns = {"/ActualizarUsrMed"})

public class ActualizarUsrMed extends HttpServlet {
        ...
        try {
            if (act.ModifUsuario(nombre, correo, apodo, correoOrig) > 0) {
                ...
                out.println("</script>");
                    ...
                    response.sendRedirect("PagsDoctor/PerfilMed.jsp");
               ...
               PrintWriter out = response.getWriter();
                ...
                out.println("window.location = 'PagsDoctor/PerfilMed.jsp';");
                out.println("</script>");
                
            }
        } catch (Exception ex) {
            Logger.getLogger(ActualizarUsrMed.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
